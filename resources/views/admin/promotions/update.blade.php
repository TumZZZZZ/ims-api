...
@section('title', __('update_promotion'))
@section('header-title', __('update_promotion'))

@php
    $data->start_date = Carbon\Carbon::parse($data->start_date)->setTimezone(getTimezone());
    $data->end_date = Carbon\Carbon::parse($data->end_date)->setTimezone(getTimezone());
@endphp
@section('content')
    ...
    <form action="{{ route('admin.promotion.update', $data->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
            ...
            <div class="col-right">
                <div style="display: flex;">
                    ...
                    <div style="width: 25%; padding-right: 20px;">
                        ...
                        <select name="type" required>
                            ...
                            @foreach ($promotion_types as $promotionType)
                                <option value="{{ $promotionType->key }}" {{ $promotionType->key == $data->type ? 'selected' : '' }}>
                    ...
                    <div style="width: 25%;">
                        ...
                        <input type="number" name="amount" value="{{ $data->value }}"
                        ...
                        <div class="custom-multi-select-wrapper" data-placeholder="{{ __('select_category') }}">
                            ...
                            <ul class="custom-multi-options">
                                @foreach ($categories as $category)
                                    <li>
                                        <input type="checkbox" name="category_ids[]" value="{{ $category->id }}" {{ in_array($category->id, $data->category_ids) ? 'checked' : '' }}>
